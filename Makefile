...
hpc-preview:
	@for DATASET_NAME in "AMRB1" "AMRB2"; do \
...
hpc-train-amrb1-kfold-strain:
	for MODEL_NAME in "resnet" "drcaps"; do \
		...
		--export=ALL,SCRIPT_NAME=2_training,CV_MODE=k-fold,LABEL_TYPE=strain,DATASET_NAME=AMRB1,MODEL_NAME=$${MODEL_NAME} jobscript.sh; \
	done

hpc-train-amrb1-kfold-species:
	for MODEL_NAME in "resnet" "drcaps"; do \
		sbatch -J qpm-amrb1_k-fold_species_$${MODEL_NAME} --array=0-9 \
		--export=ALL,SCRIPT_NAME=2_training,CV_MODE=k-fold,LABEL_TYPE=species,DATASET_NAME=AMRB1,MODEL_NAME=$${MODEL_NAME} jobscript.sh; \
	done

hpc-train-amrb1-kfold-type:
	for MODEL_NAME in "resnet" "drcaps"; do \
		sbatch -J qpm-amrb1_k-fold_type_$${MODEL_NAME} --array=0-9 \
		--export=ALL,SCRIPT_NAME=2_training,CV_MODE=k-fold,LABEL_TYPE=type,DATASET_NAME=AMRB1,MODEL_NAME=$${MODEL_NAME} jobscript.sh; \
	done

hpc-train-amrb1-kfold-gram:
	for MODEL_NAME in "resnet" "drcaps"; do \
		sbatch -J qpm-amrb1_k-fold_gram_$${MODEL_NAME} --array=0-9 \
		--export=ALL,SCRIPT_NAME=2_training,CV_MODE=k-fold,LABEL_TYPE=gram,DATASET_NAME=AMRB1,MODEL_NAME=$${MODEL_NAME} jobscript.sh; \
	done
...
hpc-train-amrb2-kfold-strain:
	for MODEL_NAME in "resnet" "drcaps"; do \
		...
		--export=ALL,SCRIPT_NAME=2_training,CV_MODE=k-fold,LABEL_TYPE=strain,DATASET_NAME=AMRB2,MODEL_NAME=$${MODEL_NAME} jobscript.sh; \
	done

hpc-train-amrb2-kfold-species:
	for MODEL_NAME in "resnet" "drcaps"; do \
		sbatch -J qpm-amrb2_k-fold_species_$${MODEL_NAME} --array=0-9 \
		--export=ALL,SCRIPT_NAME=2_training,CV_MODE=k-fold,LABEL_TYPE=species,DATASET_NAME=AMRB2,MODEL_NAME=$${MODEL_NAME} jobscript.sh; \
	done

hpc-train-amrb2-kfold-type:
	for MODEL_NAME in "resnet" "drcaps"; do \
		sbatch -J qpm-amrb2_k-fold_type_$${MODEL_NAME} --array=0-9 \
		--export=ALL,SCRIPT_NAME=2_training,CV_MODE=k-fold,LABEL_TYPE=type,DATASET_NAME=AMRB2,MODEL_NAME=$${MODEL_NAME} jobscript.sh; \
	done
...
hpc-train-amrb1-leaveout-strain:
	for MODEL_NAME in "resnet" "drcaps"; do \
		...
		--export=ALL,SCRIPT_NAME=2_training,CV_MODE=leave-out,CV_FOLDS=5,LABEL_TYPE=strain,DATASET_NAME=AMRB1,MODEL_NAME=$${MODEL_NAME} jobscript.sh; \
	done

hpc-train-amrb1-leaveout-species:
	for MODEL_NAME in "resnet" "drcaps"; do \
		sbatch -J qpm-amrb1_leave-out_species_$${MODEL_NAME} --array=0-2 \
		--export=ALL,SCRIPT_NAME=2_training,CV_MODE=leave-out,CV_FOLDS=3,LABEL_TYPE=species,DATASET_NAME=AMRB1,MODEL_NAME=$${MODEL_NAME} jobscript.sh; \
	done
...
hpc-train-amrb2-leaveout-strain:
	for MODEL_NAME in "resnet" "drcaps"; do \
		...
		--export=ALL,SCRIPT_NAME=2_training,CV_MODE=leave-out,CV_FOLDS=19,LABEL_TYPE=strain,DATASET_NAME=AMRB2,MODEL_NAME=$${MODEL_NAME} jobscript.sh; \
	done

hpc-train-amrb2-leaveout-species:
	for MODEL_NAME in "resnet" "drcaps"; do \
		sbatch -J qpm-amrb2_leave-out_species_$${MODEL_NAME} --array=0-3 \
		--export=ALL,SCRIPT_NAME=2_training,CV_MODE=leave-out,CV_FOLDS=4,LABEL_TYPE=species,DATASET_NAME=AMRB2,MODEL_NAME=$${MODEL_NAME} jobscript.sh; \
	done
