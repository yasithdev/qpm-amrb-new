...
if __name__ == "__main__":
    ...
    # K-Fold - Species
    results_kf_sp = df_kf_sp.groupby("model")[cols].agg(["mean", "std"]).T
    print("AMRB2 K-Fold (Species)")
    print(results_kf_sp)

    # K-Fold - Strain
    results_kf_st = df_kf_st.groupby("model")[cols].agg(["mean", "std"]).T
    print("AMRB2 K-Fold (Strain)")
    print(results_kf_st)

    # Leave-Out - Strain
    results_lo_sp = df_lo_sp.groupby("model")[cols].agg(["mean", "std"]).T
    print("AMRB2 Leave-Out (Species)")
    print(results_lo_sp)

    # Leave-Out - Strain
    results_lo_st = df_lo_st.groupby("model")[cols].agg(["mean", "std"]).T
