...
def main():
  ...
  for label in labels:
    ...
    for imag in tqdm(images, desc=f'Label: {label}', **tqdm_args):
      ...
      if c_num > 0:
        ...
        if img_l >= img_d:  # case - exact/undersized: zero-pad around the object
          a = (img_l - img_d) // 2
          b = img_l - img_d - a
          ...
          if c_num == 1:  # accept
            ...
            imag_accepted.append(final_imag * final_mask)  # masking noise (if any)
          else:  # reject
  ...
  np.savez_compressed(
    os.path.join(experiment_path, "stats.npz"), **ds_stats
  )
  print("DONE")
